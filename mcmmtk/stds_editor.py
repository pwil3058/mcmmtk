...
class ModelPaintStandardEditor(standards.PaintStandardEditor):
    PAINT_EDITOR = ModelPaintEditor
    PAINT_LIST_NOTEBOOK = ModelPaintListNotebook

recollect.define(ModelPaintStandardEditor.RECOLLECT_SECTION, "last_geometry", recollect.Defn(str, ""))
...
class TopLevelWindow(dialogue.MainWindow):
    ...
    def __init__(self):
        ...
        self.parse_geometry(recollect.get(ModelPaintStandardEditor.RECOLLECT_SECTION, "last_geometry"))
        self.set_icon_from_file(icons.APP_ICON_FILE)
        self.editor = ModelPaintStandardEditor()
    ...
    def _file_changed_cb(self, widget, file_path):
        self.set_title(_('mcmmtk: Paint Standard Editor: {0}').format(file_path))
    def _configure_event_cb(self, widget, event):
        recollect.set(ModelPaintStandardEditor.RECOLLECT_SECTION, "last_geometry", "{0.width}x{0.height}+{0.x}+{0.y}".format(event))
